import { readFile } from "node:fs/promises";

const requiredFiles = [
  "界面原型-v1/index.html",
  "界面原型-v1/styles.css",
  "界面原型-v1/local-icons.js",
  "界面原型-v1/archive-config.js",
  "界面原型-v1/archive-data.js",
  "界面原型-v1/archive-index-data.js",
  "界面原型-v1/archive-batch-data.js",
  "界面原型-v1/archive-batch-history-data.js",
  "界面原型-v1/app.js",
  "server.mjs",
  "scan-local-index.mjs",
  "scan-selected-folder.mjs",
  "merge-indexes.mjs",
  "verify-local-demo.mjs",
  "backup-index.mjs",
  "health-check.mjs",
  "start-demo.mjs",
  "open-local-app.mjs",
  "演示版使用说明.md",
  "演示路线卡.md",
  "单机版交付清单.md",
  "真实硬盘接入SOP.md",
  "多源存储接入与同步方案.md",
  "项目进度看板.md",
  "项目进度看板.html",
];

const requiredText = [
  [
    "界面原型-v1/index.html",
    [
      "local-icons.js",
      "archive-config.js",
      "archive-data.js",
      "archive-index-data.js",
      "archive-batch-data.js",
      "archive-batch-history-data.js",
      "archive-merged-index-data.js",
      "app.js",
      "颜色调色盘",
      "界面设置",
      "快速调色板",
      "动态资讯字幕",
      "infoTickerTrack",
      "infoTickerFilters",
      "黑卫士 AI 档案馆",
      "开发者：Alex",
      "菜单位置",
      "menuPositionControls",
      "左侧菜单显示方式",
      "menuDisplayModeControls",
      "隐形悬停",
      "menuCollapseToggle",
      "档案全网总控闭环",
      "archiveCommandFlow",
      "searchModeGrid",
      "authorizationGateGrid",
      "queryBillingGrid",
      "productEditionGrid",
      "公司的组织架构",
      "orgChartSearchInput",
      "companyOrgGrid",
      "historicalOrgImportGrid",
      "历史组织架构图",
      "人事部门",
      "数据库快捷分类",
      "archiveQuickCategoryGrid",
      "archiveQuickAddButton",
      "archiveQuickShowHidden",
      "本机索引快搜",
      "localIndexSearchInput",
      "local-shortcut",
      "全介质采集入口",
      "intakeSourceGrid",
      "intakeSelected",
      "intakeWorkflow",
      "storageStatusGrid",
      "downloadDestinationGrid",
      "downloadProtectionGrid",
      "sensitiveMaskGrid",
      "dailyLedgerSummary",
      "dailyLedgerBody",
      "受控下载与外发治理",
      "每日台账",
      "本地下载",
      "公司工作邮箱外发",
      "敏感数字打格子",
      "batchHistoryList",
      "批次历史看板",
      "sampleValidationList",
      "sampleValidationForm",
      "samplePreflightList",
      "sampleDecisionList",
      "roleSelect",
      "moduleToggleGrid",
      "permissionMatrix",
      "secondaryMenuGrid",
      "fileTypeLibrary",
      "formulaDemoGrid",
      "真实介质样本验证",
      "组织架构底座",
      "权限视角",
      "电子文件类型库",
      "Demo 公式样例",
      "数据来源总库",
      "sourceVaultCategoryGrid",
      "dataRepositorySeries",
      "annualBackupPolicyGrid",
      "videoIncrementalRuleGrid",
      "视频大文件增量规则",
      "手机",
      "iPad",
      "旧电脑",
      "邮箱",
      "云盘",
      "NAS",
      "U盘",
      "SD卡",
      "相机",
      "摄像机",
      "NAS 暂存区",
    ],
  ],
  [
    "界面原型-v1/app.js",
    [
      "localIndexMatches",
      "clearLocalSearch",
      "local-shortcut",
      "completionScore",
      "qualityScore",
      "aiRepairLabel",
      "AI 修复候选",
      "creationTool",
      "contentIntro",
      "wordCountText",
      "structureText",
      "nodeText",
      "purposeText",
      "contributionMode",
      "selfCheckScore",
      "renderInfoTicker",
      "renderStorageStatus",
      "renderDownloadGovernance",
      "renderDailyLedger",
      "renderSourceVault",
      "renderArchiveQuickCategories",
      "addArchiveQuickCategory",
      "handleArchiveQuickAction",
      "archiveQuickCategories",
      "archiveQuickCustom",
      "archiveQuickHidden",
      "授权打开",
      "renderCompanyOrgChart",
      "orgDocumentStats",
      "orgDocumentQuery",
      "companyOrgGrid",
      "orgChartSearchInput",
      "historicalOrgImportGrid",
      "storageUsedPercent",
      "activeTickerCategory",
      "applyMenuLayout",
      "menuPositionControls",
      "controlledViewingRules",
      "自适应预览",
      "受控浏览策略",
      "防偷拍",
      "摄像头",
      "人脸识别",
      "鼠标行为",
      "修改次数",
      "修改人",
      "修改电脑",
      "MAC地址",
      "IP地址",
      "修改坐标",
      "不可浏览",
      "isEncryptedImportantFile",
      "encrypted-important-file",
      "红色 · 需加密/受控",
    ],
  ],
  [
    "界面原型-v1/app.js",
    [
      "periodSearchText",
      "assetStage",
      "qualityLevel",
      "securityLevelName",
      "storageSourceType",
      "accessMode",
      "syncStrategy",
      "crossCheckPolicy",
      "storageRisk",
      "storageCostLevel",
      "L6 绝密",
      "真实接入批次控制",
      "扫描前确认",
      "扫描报告",
      "AI 候选边界",
      "renderBatchHistory",
      "renderSampleValidation",
      "HWS_SCAN_BATCH_HISTORY",
      "HWS_MERGED_ARCHIVE_INDEX",
    ],
  ],
  [
    "scan-local-index.mjs",
    [
      "本机只读扫描完成",
      "archive-index.json",
      "HWS_LOCAL_ARCHIVE_INDEX",
      "L0 外部流通",
      "L6 绝密",
      "第三方云盘",
      "局域网网盘/NAS",
      "U盘/移动硬盘",
      "SD卡",
      "相机",
      "摄像机",
      "手机",
      "iPad",
      "邮箱",
      "云盘",
      "NAS 暂存区",
      "accessMode",
      "crossCheckPolicy",
      "storageRisk",
      "storageCostLevel",
      "truncated",
      "skippedEntries",
      "archive-batch-history-data.js",
      "sanitizeArchiveForBrowser",
      "hasLocalPath",
    ],
  ],
  ["scan-selected-folder.mjs", ["只读扫描入口", "choose folder", "不移动、不删除、不改名", "open-local-app.mjs"]],
  ["merge-indexes.mjs", ["多目录合并索引", "duplicateCount", "HWS_MERGED_ARCHIVE_INDEX", "sanitizeArchiveForBrowser"]],
  ["界面原型-v1/local-icons.js", ["smartphone", "sd-card", "camera"]],
  [
    "verify-local-demo.mjs",
    [
      "单机版验收报告",
      "格式分布",
      "作品类型分布",
      "完整状态分布",
      "作品等级分布",
      "存储来源分布",
      "接入方式分布",
      "同步策略分布",
      "交叉校验分布",
      "成本等级分布",
      "扫描告警",
      "浏览器索引使用脱敏数据",
      "复制相对路径",
    ],
  ],
  ["backup-index.mjs", ["备份快照", "snapshot-manifest.json"]],
  ["health-check.mjs", ["健康检查通过", "/api/health", "浏览器脱敏索引"]],
  ["start-demo.mjs", ["只读扫描并生成本机索引", "生成单机验收报告", "检查演示版完整性"]],
  ["open-local-app.mjs", ["桌面入口", "findRunningServer", "acquireStartLock", "项目进度看板.html"]],
  ["server.mjs", ["/api/health", "/api/reveal", "indexedPathById", "searchParams.get(\"id\")", "open"]],
  ["界面原型-v1/archive-index-data.js", ["HWS_LOCAL_ARCHIVE_INDEX", "archives", "hasLocalPath"]],
  ["界面原型-v1/archive-batch-data.js", ["HWS_LATEST_SCAN_BATCH", "batchId", "scanPolicy", "aiGate"]],
  ["界面原型-v1/archive-batch-history-data.js", ["HWS_SCAN_BATCH_HISTORY", "batchId", "scanPolicy", "aiGate"]],
  [
    "界面原型-v1/archive-config.js",
    [
      "组织项目检索",
      "电子文件类型",
      "secondaryMenus",
      "roles",
      "modules",
      "permissionMatrix",
      "fileTypeGroups",
      "formulaExamples",
      "公司组织架构",
      "archiveQuickCategories",
      "老板个人档案",
      "家庭档案",
      "公司档案",
      "在职档案",
      "离职档案",
      "经典作品",
      "合同案例",
      "公司的组织架构",
      "companyOrgChart",
      "companyOrgStructures",
      "historicalOrgChartImports",
      "历史组织架构图",
      "人事部门",
      "按部门查文档",
      "infoTickerCategories",
      "infoTickerItems",
      "storageEndpoints",
      "downloadDestinations",
      "downloadProtectionRules",
      "sensitiveMaskRules",
      "dailyLedgerItems",
      "本地电脑",
      "公司指定工作邮箱",
      "水印",
      "打格子",
      "至少 3 人审批",
      "至少 2 名最高授权人",
      "每日台账",
      "平台动态",
      "行业资讯",
      "企业资讯",
      "岗位资讯",
      "制度资讯",
      "权限与审计",
      "系统设置",
      "公司类型",
      "周期阶段",
      "作品等级",
      "存储来源",
      "接入方式",
      "同步策略",
      "交叉校验",
      "成本等级",
      "方案/提案",
      "标书/投标/招采",
      "intakeSources",
      "intakeWorkflow",
      "sampleValidationChecklist",
      "sampleValidationFields",
      "samplePreflightChecks",
      "sampleResultDecisions",
      "sourceVaultCategories",
      "sourceVaultInterfaces",
      "dataRepositorySeries",
      "annualIncrementalBackupPolicies",
      "videoIncrementalRules",
      "原始数据增量库",
      "只增量，不减少",
      "1G 以上",
      "人工分类",
      "视频冷存储池",
      "手机",
      "iPad",
      "旧电脑",
      "邮箱",
      "云盘",
      "NAS",
      "U盘",
      "SD卡",
      "相机",
      "摄像机",
      "NAS 暂存区",
      "batchPrefix",
      "接入前登记",
      "只读试扫",
      "风险复核",
      "样本来源类型",
      "隐私排除",
      "只读原则",
      "路径脱敏",
      "点验结果判定表",
      "扩大下一批",
      "scanScope",
      "securityDefault",
      "precheck",
      "report",
      "nasGate",
      "aiGate",
      "smartphone",
      "sd-card",
      "camera",
      "人脸识别",
      "controlledViewingRules",
      "archiveCommandFlow",
      "searchModes",
      "authorizationGates",
      "queryBillingRules",
      "productEditions",
      "系统设置",
      "加密文件策略",
      "核心参数文件",
      "积分兑换",
      "付费",
      "企业家版",
      "公司版",
      "双人共同查看",
      "导入必须查毒",
      "防恶意拍照",
      "普通受控",
      "最高机密",
      "绝密",
      "全文浏览",
      "带底纹浏览",
      "不可浏览",
      "公众浏览防偷拍",
      "修改次数",
      "修改人",
      "修改电脑",
      "MAC地址",
      "IP地址",
      "修改坐标",
      "appearance",
      "完成度",
      "水准",
      "AI 修复",
    ],
  ],
  [
    "界面原型-v1/archive-data.js",
    [
      "某汽车品牌发布会策划案",
      "体育赛事运营服务投标文件",
      "客户往来邮件归档",
      "年度经营数据电子表格",
    ],
  ],
  ["项目进度看板.md", ["甘特图", "1 天单机可用版", "每完成一个工作项", "并行模块总控表"]],
  ["项目进度看板.html", ["项目管理看板", "甘特图", "进度表", "并行模块总控", "progress-badge", "pulse-normal"]],
  ["演示路线卡.md", ["3 分钟演示路线", "本机索引快搜", "验收口径", "多源存储检索"]],
  ["单机版交付清单.md", ["启动命令", "验收命令", "交付文件", "多源存储"]],
  [
    "真实硬盘接入SOP.md",
    [
      "只读接入",
      "扫描前确认",
      "不移动、不删除、不改名",
      "模糊时间段的周期搜索",
      "L6 绝密",
      "多源接入范围",
      "真实接入批次",
      "真实介质样本验证",
      "真实样本点验登记模板",
      "真实样本扫描前检查清单",
      "点验结果判定表",
      "禁训清单",
      "批次编号",
      "扫描报告",
      "AI 候选边界",
    ],
  ],
  [
    "多源存储接入与同步方案.md",
    [
      "在线硬盘",
      "云端云盘",
      "局域网网盘",
      "NAS",
      "不同主机硬盘",
      "U盘",
      "接入方式",
      "同步策略",
      "交叉校验",
      "成本等级",
      "风险等级",
    ],
  ],
];

for (const file of requiredFiles) {
  await readFile(file, "utf8");
}

for (const [file, needles] of requiredText) {
  const content = await readFile(file, "utf8");
  for (const needle of needles) {
    if (!content.includes(needle)) {
      throw new Error(`${file} 缺少：${needle}`);
    }
  }
}

const html = await readFile("界面原型-v1/index.html", "utf8");
if (/https?:\/\//.test(html)) {
  throw new Error("index.html 仍包含外网依赖，不适合作为离线演示版。");
}

const browserIndex = await readFile("界面原型-v1/archive-index-data.js", "utf8");
if (/"path"\s*:/.test(browserIndex) || /\/Users\/|\/Volumes\/|[A-Za-z]:\\/.test(browserIndex)) {
  throw new Error("浏览器索引仍包含本机完整路径，必须先脱敏。");
}

try {
  const privateIndex = JSON.parse(await readFile("界面原型-v1/archive-index.json", "utf8"));
  if (!("truncated" in privateIndex) || !("skippedCount" in privateIndex) || !("skippedEntries" in privateIndex)) {
    throw new Error("本机索引缺少扫描上限或跳过条目元数据。");
  }
} catch (error) {
  if (error.code !== "ENOENT") {
    throw error;
  }
}

console.log("演示版检查通过：文件完整、核心入口完整、无外网依赖。");
