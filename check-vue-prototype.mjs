...
const requiredText = [
  ...
  ["界面原型-vue/src/components/DocumentPreview.vue", ["selectedDocument.controls", "control.includes", "授权查看", "分级导出", "预览授权策略联动", "授权前置条件", "预览安全边界", "预览授权台账", "仅模拟授权不打开真实文件", "simulatePreviewAuthorization", "preview-audit", "仅记录模拟审计不触碰真实文件", "摄像头/人脸/水印策略配置化", "摄像头策略", "人脸识别策略", "水印策略", "策略台账", "模拟切换", "controlPolicyList", "toggleControlPolicy", "仅模拟策略不调用摄像头/人脸识别"]],
  ...
  ["界面原型-vue/src/components/ImportExportGovernance.vue", ["导入必须查毒", "导出分级授权", "双人授权", "台账留痕", "执行步骤", "审批门禁", "治理台账", "仅模拟不执行真实动作", "simulateGovernanceStep"]],
  ...
  ["界面原型-vue/src/components/AuditLedgerPanel.vue", ["审计详情台账", "动作类型", "操作者", "密级", "审批状态", "actionFilter", "filteredEntries", "查看", "下载", "导出", "授权打开", "预览授权审计联动", "仅记录模拟审计不触碰真实文件"]],
  ["界面原型-vue/src/components/ProgressSnapshot.vue", ["Vue 重构", "响应式", "1500 行"]],
  ["界面原型-vue/src/styles/responsive.css", ["@media", "max-width: 900px", "max-width: 390px", "grid-template-columns", "overflow-wrap", "permission-ledger", "audit-filter-row", "settings-policy-grid", "governance-ledger", "preview-policy-grid", "preview-audit-ledger", "control-policy-grid", "control-policy-ledger"]],
];
